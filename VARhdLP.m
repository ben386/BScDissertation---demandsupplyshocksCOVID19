function [HD, VAR] = VARhdLP(VAR,VARopt, weight)
% =======================================================================
...
if size(weight, 1) == 1
    weightdim = size(weight, 2);
else
    weightdim = size(weight, 1);
end

covpar = ones(1, (size(VAR.Y, 1) - weightdim)); % ROW vector of ones

if size(weight, 1) == 1
    covpar = [covpar, weight];
else
    covpar = [covpar, transpose(weight)];
end
%covpar is a row vector of the COVID parameters
...
Y       = VAR.Y .* transpose(covpar);     % left-hand side
X       = VAR.X(:,1+const:nvarXeq+const) .* transpose(covpar); % right-hand side (no exogenous)
...
eps = B\transpose(VAR.resid .* transpose(covpar)); % structural errors 
